...
for fsi_result_file in fsi_result_paths:
    with open(os.path.join(fsi_result_path, fsi_result_file), 'rb') as f:
        addrs, pred_scores, infer_time = CPU_Unpickler(f).load()
        fsi_results[addrs] = (pred_scores, infer_time)
        # pred_scores[i]:  {addr->score}
...
def analysis(job_index, index, contract_address, result_queue):
    try:
        ...
        func_boundary, start_pcs, instr_seq, analy_time = function_boundary_detection(binary_full_path, fsi_result[0])
        entry_golden, fb_golden, n = load_ground_truth(ground_path)
        ...
        with open(os.path.join(result_path, contract_address), "wb") as f:
            _instr_seq = []
            for instr in instr_seq:
                _instr_seq.append((instr.name, instr.address, instr.tag_id))
            pickle.dump((func_boundary, _instr_seq), f)

        print("{}: {} : {} sec".format(index, contract_address, fsi_result[1] + analy_time))
        result_queue.put(
            (contract_address, [], {"fs": fs_results1, "fb": fb_results, "time1": fsi_result[1], "time2": analy_time}))

    except Exception as e:
        log("{}: {} exception: {}..".format(index, contract_address, str(e)))
