...
class Instruction(object):
    ...
    def __init__(self, opcode, name, length_of_operand=0, description=None, args=None, returns=None, gas=-1, category=None, pops=None, pushes=None, fork=''):
        ...
        self.next = None
        self.previous = None
        self.is_tag = False  # True if the current instruction is a PUSH and the operand is a pc address (can be mapped to a tag) 
    ...
    def consume(self, bytecode):
        ...
        if not (len(m.operand_bytes) == m.length_of_operand):
            #print("no enough operand bytes: "+str(len(m.operand_bytes))+" "+str(m.length_of_operand))
            #assert(False)
            pass
        return m
